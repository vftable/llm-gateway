...
test("R2: messages->chat inserts a placeholder tool reply for an unanswered tool_use", () => {
  ...
  const msgs = out.messages as Array<{ role: string; tool_call_id?: string; content?: unknown }>;
...
test("S1 streaming: chat reasoning_content -> Anthropic thinking events", async () => {
  ...
  assert.equal((thinkingDelta!.data.delta as { thinking: string }).thinking, "th");
});
  ...
  const out = chatRequestToMessages({
    ...
    messages: [
      { role: "user", content: "call it" },
      {
        ...
        content: "sure",
        tool_calls: [
          { id: "toolu_a", type: "function", function: { name: "f", arguments: '{"x":1}' } },
        ],
      },
