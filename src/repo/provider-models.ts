...
function parseTransforms(raw: string): ModelTransformConfig[] {
  try {
    const parsed = JSON.parse(raw);
    if (Array.isArray(parsed)) return parsed as ModelTransformConfig[];
  } catch {
    /* fall through */
  }
  return [];
...
function mapProviderModel(r: ProviderModelRow): ProviderModel {
  return {
    ...
    transforms: parseTransforms(r.transforms),
    notes: r.notes,
...
export function countProviderModelsByProvider(
  db: DB,
...
export function getProviderModelById(
  ...
  const row = db.prepare("SELECT * FROM provider_models WHERE id = ?").get(id) as
  ...
  db.prepare(
    `INSERT INTO provider_models
       (provider_id, upstream_id, display_name, context_window,
        max_output_tokens, transforms, notes, created_at, updated_at)
...
export function updateProviderModel(
  ...
  db.prepare(
    `UPDATE provider_models SET
       ...
       max_output_tokens=@max_output_tokens, transforms=@transforms,
